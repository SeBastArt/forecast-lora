...
$(window).on("load", function() {
   $("canvas[id^='simpleLineChart']").each(function (i, el) {
       ...
       setTimeout(() => {  UpdateChartJSData(simpleLineChart, nodeId); }, 250);
   });

   $("div[id^='shadowLineChart']").each(function (i, el) {
      let nodeId = el.id.split("-")[1];
      let shadowLineChart = CreateShadowLineChart(el.id, nodeId);
      setTimeout(() => {  UpdateChartistData(shadowLineChart, nodeId); }, 250);
  });
 }); 
...
{
   console.log(window.location.origin + '/nodes/' + nodeId + '/data');
   $.ajax({
      url : window.location.origin + '/nodes/' + nodeId + '/data',
      type : 'GET', 
      data : {
         //'numberOfWords' : 10
      },
