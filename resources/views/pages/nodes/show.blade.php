         ...
         <form method="POST" action="{{action('Web\NodeController@update', ['node' => $Node->id])}}">
            ...
            <div id="inline-form" class="">
               <div class="row">
                  ...
                  <div class="input-field col s12">
                     <input id="InputDevEui" name="dev_eui" type="text" class="validate" value="{{$Node->dev_eui}}">
                     <label for="InputDevEui">DevEui</label>
                  </div>
                  <div class="input-field col s12">
                     <select name="nodetype">
                     <option value="" disabled selected>Choose node type</option>
                     <option value="1" {{$Node->node_type_id === 1 ? 'selected="selected"' : ''}}>Decentlab</option>
                     <option value="2" {{$Node->node_type_id === 2 ? 'selected="selected"' : ''}}>Cayenne</option>
                     <option value="3" {{$Node->node_type_id === 3 ? 'selected="selected"' : ''}}>Dragino</option>
                     <option value="4" {{$Node->node_type_id === 4 ? 'selected="selected"' : ''}}>Zane</option>
                     </select>
      ...
      <ul class="collapsible todo-collection" id = "sortable">
         ...
         <li id="{{$field->position}}">
            <div class="collapsible-header">
               ...
               <span class="badge z-depth-2" style="background-color: {{$field->primarycolor}} !important; color: {{$field->secondarycolor}}" data-badge-caption="color-scheme"></span>
            </div>
                  ...
                  <div id="inline-form" class="">
                     ...
                     <h4 class="card-title">Add Field to {{$Node->name}} Node</h4>
                        <div class="row">
                           ...
                           <div class="input-field col s12">
                              <button class="btn waves-effect waves-light right" type="submit" name="action">Add Field
